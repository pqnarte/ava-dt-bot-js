...
const url = 'https://ava-dog-tag.wikia.com/wiki/Category:Maps';

module.exports.run = async (bot, message, args) => {
  const max_time_ms = 30000;
  if (args.length <= 0){
    message.channel.send("You need the map name!").then(msg => {
    ...
    await rp(options)
      ...
      if (data.length > 1){
        message.channel.send("Found several maps! Choose one!").then(async msg => {
          text = "";
                ...
                await rp(options)
                  .then(($) => {
                    ...
                    var content = $('.mw-content-ltr').find('figure').each(function(i,elem) {
                      image_url = $(this).children().attr('href').attr('src');
                    });
